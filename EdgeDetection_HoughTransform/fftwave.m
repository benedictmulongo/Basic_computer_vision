function  [] = fftwave(u, v, sz)
  ...
  wavelength = sz / sqrt(u^2 + v^2);  % Replace by correct expression 
  amplitude = max(abs(F(:))) / (sz^2);   % Replace by correct expression

  subplot(3, 2, 2);
  showgrey(fftshift(Fhat));
  title(sprintf('centered Fhatpp: (uc, vc) = (%d, %d)', uc, vc))
